...
function buildHomeSlide() {
  ...
  while (i < bannerPositonIdArray.length && flag < 1) {
    ...
    let altBannerMasc = document.querySelector(
      `#homeSlideForm > section:nth-child(${bannerPositonId}) > div:nth-child(5) > input[type=text]`
    ).value;

    //Choosing HTML element
    let resultLinkBannerMasc = document.querySelector(
      `.pdv-automation div:nth-child(${bannerPositonId}) > a`
    );
    let resultBannerDeskMasc = document.querySelector(
      `.pdv-automation div:nth-child(${bannerPositonId}) > a > picture > img`
    );
    let resultBannerMobileMasc = document.querySelector(
      `.pdv-automation div:nth-child(${bannerPositonId}) > a > picture > source`
    );
