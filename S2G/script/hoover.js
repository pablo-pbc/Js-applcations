...
function buildHtml() {
    ...
    for (i = 0; i < nodeList.length && flag < 1; i++) {
      ...
      if (linkHoover.length < 3 || imgHoover.length < 3 || altHoover.length < 3) {
        ...
        resultHoover.style.display = 'none'
        ...
        resultHoover.innerHTML =`<img src=${finalUrl} alt=${altHoover} border="0" />${altHoover}` ;
      }
    };
    ...
    for (let i = 0; i < nodeList.length; i++) {
        ...
        let inputHtmlSection = `
                ...
                <div class='input_checkbox'>
                    <label>Marque aqui, caso queira remover este Hoover!</label>
                    <input type="checkbox">                  
                </div>
                </section>
            `;
